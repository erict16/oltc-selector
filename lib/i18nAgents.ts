...
const zh: AgentGuideCopy = {
  ...
  how: "在助手里找上传技能的入口，WorkBuddy 里叫 Upload skill，别的助手也有类似的入口。把 zip 传进去就行。之后把变压器参数发给它就行，有什么参数不确定，它会来问你。",
  dl: "下载技能包",
  cli: "想自己跑命令",
  cliBody:
    "技能背后是个命令行工具 oltc，和网页版同一套选型逻辑。装上它，把电流、电压喂给它，再对照样本册。它不编型号，样本册里没有的就是没有。",
  install: "安装",
  run: "有载例子",
  runNote:
...
const en: AgentGuideCopy = {
  ...
  how: "Find the skill upload entry in your assistant. In WorkBuddy it is called Upload skill; other assistants have something similar. Send the zip in. Then just send it the transformer data. If a parameter is unclear, it will ask.",
  dl: "Download the skill pack",
  cli: "Want to run it yourself",
  cliBody:
    "Behind the skill is a small command line tool, oltc, with the same selection logic as the web app. Install it, feed it the current and voltage, then check the brochure. It never invents a type. Not in the brochure means it does not exist.",
  install: "Install",
  run: "On-load example",
  runNote:
...
const vi: AgentGuideCopy = {
  ...
  how: "Tìm mục tải skill trong trợ lý: trong WorkBuddy là Upload skill, trợ lý khác có mục tương tự. Gửi file zip vào. Sau đó cứ gửi số liệu máy biến áp cho nó. Có thông số nào không rõ, nó sẽ hỏi lại bạn.",
  dl: "Tải gói skill",
  cli: "Muốn tự chạy lệnh",
  cliBody:
    "Phía sau skill là công cụ dòng lệnh oltc, cùng logic chọn với bản web. Cài nó, đưa dòng điện và điện áp vào, rồi đối catalogue. Nó không bịa kiểu. Không có trong catalogue nghĩa là không có.",
  install: "Cài đặt",
  run: "Ví dụ có tải",
  runNote:
...
const es: AgentGuideCopy = {
  ...
  how: "Busque la opción de subir skills en su asistente: en WorkBuddy se llama Upload skill; otros asistentes tienen algo parecido. Envíe el zip. Luego envíele los datos del transformador. Si algún parámetro no está claro, se lo preguntará.",
  dl: "Descargar el skill",
  cli: "Si quiere ejecutarlo usted mismo",
  cliBody:
    "Detrás del skill hay una herramienta de línea de comandos, oltc, con la misma lógica de selección que la web. Instálela, pásle la corriente y la tensión y compruebe el folleto. No inventa tipos. Si no está en el folleto, no existe.",
  install: "Instalar",
...
const tr: AgentGuideCopy = {
  ...
  how: "Asistanınızda skill yükleme girişini bulun: WorkBuddy'de buna Upload skill denir, diğer asistanlarda da benzeri vardır. Zip'i gönderin. Sonra trafo verilerini göndermeniz yeterli. Emin olmadığı bir parametre olursa size sorar.",
  dl: "Skill paketini indir",
  cli: "Kendiniz çalıştırmak isterseniz",
  cliBody:
    "Skill'in arkasında web ile aynı seçim mantığına sahip bir komut satırı aracı var: oltc. Kurun, akımı ve gerilimi verin, broşürle kontrol edin. Tip uydurmaz. Broşürde yoksa yoktur.",
  install: "Kurulum",
...
const ru: AgentGuideCopy = {
  ...
  how: "Найдите в помощнике загрузку skill: в WorkBuddy это Upload skill, у других есть похожая функция. Отправьте zip. Дальше просто отправьте данные трансформатора. Если какой-то параметр непонятен, он спросит.",
  dl: "Скачать skill",
  cli: "Если хотите запускать сами",
  cliBody:
    "Внутри skill командная утилита oltc с той же логикой подбора, что и на сайте. Установите её, передайте ток и напряжение, затем сверьте с брошюрой. Она не выдумывает типы. Нет в брошюре, значит нет.",
  install: "Установка",
