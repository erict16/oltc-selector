...
import { throughCurrentFromRated } from "./deriveUm";
...
describe("catalogue menu labels", () => {
  ...
  it("prints the catalogue number and unit, with no ≤ ≥ >", () => {
    ...
    expect(UM_MENU.find((x) => x.value === 72.5)?.labelZh).toBe("72.5 kV");
    expect(UM_MENU.find((x) => x.value === 40.5)?.labelEn).toBe("40.5 kV");
    expect(UM_MENU.find((x) => x.value === 126)?.labelEn).toBe("126 kV");
    expect(UM_MENU.find((x) => x.value === 72.5)?.labelEn).not.toMatch(/[()（）]/);
