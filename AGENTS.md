...
Result card **开关结构** (not 工作位置): CV/CV2/SV/CVT/CZ → 复合式; CM/CM2/SHZV/SDZV/CMD/HWV → 组合式; WSL/WDL → 笼式; WSG → 鼓式. Positions stay in the type string (`10193W` = 19 mech / 17 transformer steps).

## CLI

CLI: `npx -y oltc-selector@1.2.9`. Same engine as the web app. No prices. `--octc`, `--structure combined|compound|cage|drum`, `--series II…VIII`. WSL/WDL existence is `lib/listIndex.ts` (keys only). Do not commit list RMB, coefficients, or the 2025 xlsx.
